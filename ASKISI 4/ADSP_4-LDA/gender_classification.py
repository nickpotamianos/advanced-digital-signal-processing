...
# Create person-based splits from the gender splits
def create_person_splits(gender_splits):
    person_splits = {"train": [], "test": []}

    # Process training data
    for item in gender_splits["train"]:
        person_id = extract_person_id(item["path"])
        if person_id:
            person_splits["train"].append({
                "path": item["path"],
                "label": person_id
            })

    # Process test data
    for item in gender_splits["test"]:
        person_id = extract_person_id(item["path"])
        if person_id:
            person_splits["test"].append({
                "path": item["path"],
                "label": person_id
            })
